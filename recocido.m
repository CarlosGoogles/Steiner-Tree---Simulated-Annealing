...
global mejor params res intentos c MIN maxEdgeSize minNeededEdges mejorRespuestaCosto;
    mejorRespuestaCosto = 10000000;
    maxEdgeSize = p.maxSize;
...
if c==0                 
   ...
   res.c = c;
   u = TempInicial(u); 
   fprintf('temperatura inicial calculada = %7.7f\n', c);
end

Recocido(u);
fprintf('Mejor punto encontrado:\n')
[costoTotal, conectividad] = costoSteiner(mejor.x);
fprintf('Costo: %7.7f\t - \t Conectividad: %7.7f\n', costoTotal, conectividad);
params.Imp('g', mejor.x);
fprintf('\n')
